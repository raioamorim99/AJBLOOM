...
// 3. Carrega favoritos do localStorage
function carregarFavoritos() {
  const json = localStorage.getItem("favoritos");
  return json ? JSON.parse(json) : [];
}

// 4. Exibe ou oculta mensagem “vazio”
function mostrarMensagemVazio(show) {
  msgVazio.style.display = show ? "block" : "none";
  favoritosContainer.style.display = show ? "none" : "grid";
}

// 5. Renderiza grid de favoritos
function renderizarFavoritos() {
  ...
  // REUTILIZE O SEU ARRAY global produtos do index.js
  // Ele precisa estar exposto em window.produtos:
  const produtos = window.produtos || [];

  favs.forEach(favId => {
    const p = produtos.find(x => x.id === favId || x === favId);
    if (!p) return;
...
// 6. Remove um favorito
function removerFavorito(id) {
  let favs = carregarFavoritos();
  favs = favs.filter(x => x !== id && x.id !== id);
