...
// Abrir/fechar submenus no menu mobile
...
// ================================
renderizarProdutos();
// EXPÕE O ARRAY GLOBAL DE PRODUTOS
...
firebase.auth().onAuthStateChanged(user => {
  const favoritosMenus = document.querySelectorAll(".menu-favoritos");
  favoritosMenus.forEach(li => {
    li.style.display = user ? "block" : "none";
  });
});
// ================================
// FIREBASE FAVORITOS FIRESTORE
// ================================
    ...
    firebase.auth().onAuthStateChanged(user => {
      ...
      favRef.get().then(docSnapshot => {
        if (docSnapshot.exists) {
          ...
          favRef.set(produto).then(() => {
            console.log("Produto favoritado no Firestore.");
            e.target.classList.add("favoritado");
            e.target.textContent = "♥";
          });
        }
...
// ================================
// MOSTRAR/OCULTAR LINK FAVORITOS NO MENU
// ================================
firebase.auth().onAuthStateChanged(user => {
  const favoritosMenus = document.querySelectorAll(".menu-favoritos");
  favoritosMenus.forEach(li => {
